...
namespace ztl
{
	...
	RegexInterpretor::RegexInterpretor(const wstring& pattern, const vector<RegexControl>& optional) :char_table(65535)
	{
	...
	{
		auto&& table = *this->machine->table;
		for(size_t i = 0; i < table.size();i++)
		{
			auto&& element = table[i];
			for(auto j = element.min; j <= element.max;j++)
			{
				char_table[j] = i;
			}
		}
